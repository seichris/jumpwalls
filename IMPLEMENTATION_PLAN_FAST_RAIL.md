...
- FAST server integrations use `@fastxyz/sdk`, and the web app uses a browser facade over the bound FastSet extension.
...
- Added FAST treasury transfer verification and treasury payout/refund submission helpers backed by `@fastxyz/sdk`.
...
- Added web-side FAST wallet integration through a browser-safe facade over `window.fastset`, plus FAST bind, FAST mode selection, and FAST request funding.
...
## Defaults
- New FAST settlement activity uses `fastUSDC`; historical rows may still display `SETUSDC`.
- FAST addresses are stored/displayed as `fast1...`.
- Legacy `set1...` input is normalized on the backend for bind verification.
- FAST v1 settles in `fastUSDC` for new traffic.
